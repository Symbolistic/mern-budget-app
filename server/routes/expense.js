...
router.post(
  ...
  (req, res) => {
    ...
    ExpenseGroup.find({ userFrom: userFrom, month: month, year: year }).exec(
      (err, groups) => {
...
router.post(
  ...
  passport.authenticate("jwt", { session: false }),
  (req, res) => {
    ExpenseGroup.findByIdAndDelete({ _id: req.body.groupID }).exec(
      (err, doc) => {
        if (err) return res.json({ success: false, err });
        return res.status(200).json({ success: true });
      }
...
router.post(
  ...
  (req, res) => {
    ...
    ExpenseEntry.findByIdAndUpdate(
      { _id: expenseEntryID },
      params,
      (err, doc) => {
        console.log(doc);
